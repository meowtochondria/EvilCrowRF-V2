...
| M4: Screen file splitting | ✅ Mostly done. Extracts by screen: settings → about_popup.dart (612) + subghz_clone_dialog.dart (530); record → record_file_list.dart (160); file_viewer → file_viewer_text_tab.dart (77); nrf → nrf_mousejack_tab.dart (355) + nrf_spectrum_tab.dart (164) + nrf_section_card.dart (55) helper; brute → bruter_protocol.dart (526) model; protopirate → protopirate_result_card.dart (608); files → files_actions.dart (236). Models extracted: nrf_target.dart (29), bruter_protocol.dart (526). Helper widgets extracted: status_bar_icons.dart (292) from status_bar_widget.dart (775→483). Net main-file reduction: -2000 lines. |
| M5: Delete BleProvider | 🔄 In progress. BleConnectionProvider (522 lines) created at lib/connection/ble_connection_provider.dart — handles scan/connect/disconnect/MTU/notify/sendBinaryCommand. Wired in main.dart: all module providers' sendCommand now delegates to BleConnectionProvider instead of BleProvider. ConnectionStateProvider updated to watch both BleConnectionProvider and BleProvider (transition). Remaining: migrate consumers (status_bar_widget, settings_screen, debug_screen, etc.) away from BleProvider, then delete BleProvider. |
