...
#include "core/BinaryProtocolHandler.h"  // IWYU pragma: keep — used in macro fallback
...
public:
    ...
    static bool handleLoadFileData(const uint8_t* data, size_t len) {
        ...
        // TRUE STREAMING: Use transport adapter's streaming method
        
#if EVILCROW_BT_MODE
        BleAdapter* adapter = BleAdapter::getInstance();
#elif EVILCROW_WIFI_MODE
        WifiAdapter* adapter = WifiAdapter::getInstance();
#else
        BinaryProtocolHandler* adapter = nullptr;
#endif
        if (adapter != nullptr) {
            adapter->streamFileData(header, headerSize, file, fileSize);
            file.close();
        } else {
            file.close();
            sendBinaryFileActionResult(7, false, 15, "Adapter not found"); // error 15=no adapter
        }
